...
- **页面助手（可折叠 Wiki 侧栏）** —— 在 Wiki 阅读视图右侧直接呼叫 Agent；当前页面自动作为上下文，每会话可手动钉多个页面，会话和写入策略与主聊天完全共享；对已存在 Wiki 页的覆盖会先弹出确认卡，征得同意后再写入
- **Mermaid 流程图渲染** — 聊天和预览中可直接渲染 Mermaid 代码块，语法错误会显示为紧凑错误卡片
- **异步审核系统** — LLM 在摄入时标记需人工判断的项，预定义操作，预生成搜索查询
- **Chrome 网页剪藏** — 一键捕获网页内容，自动摄入知识库
- **外部 MCP Client** — 让 LLM Wiki 作为 MCP client 连接本地 stdio MCP 服务；自带 **MiniMax Token Plan** 模板（`uvx minimax-coding-plan-mcp -y`），发现到的工具以稳定命名 `mcp.<server>.<tool>` 与内置工具并列开放给 Agent，每个 run 独立维护会话、超时、输出截断与子进程回收
- **实时搜索 + Wiki 顶部快速搜索** — 搜索视图改为输入即搜（300ms debounce + token 防过期响应覆盖）；Wiki 预览头部多了一个快速搜索框，Enter 打开第一条建议，无结果时自动跳转完整搜索
- **Wiki 删除按钮** — Wiki 编辑器顶部在 Link / Edit 之间新增「删除」按钮，仅能删除 `<project>/wiki/` 下的文件，成功后刷新文件树并关闭预览
...
- **可折叠 Wiki 右栏（页面助手）** —— Wiki 阅读视图下，右侧栏可切换为“页面助手”或 Deep Research 并互斥；页面助手自动附加当前页、可钉多个上下文页、与主聊天共享会话和流；右栏拖拽宽度沿用既有逻辑，中心 Wiki 预览不再嵌入聊天
...
- **工具型 Agent** —— 可自主选择 Wiki 检索、Source 检索、图谱检索、网页搜索、AnyTXT、workspace 文件工具、已批准的 shell 命令（开启 Enhanced shell mode 后，`python`、`rg`、`git`、`cat`、`node`、`npm`、`cargo` 等常用开发工具免审批直通），以及 Skill 文件读取
...
- **Enhanced shell mode（默认开启）** —— 让 Agent 像 Claude Code / Cursor 那样直接跑代码：`python -c "import inspect; ..."`、`rg <symbol>`、`git log` 等无需每次弹窗；网络客户端（`curl`/`wget`/`ssh`/`scp`）、提权、破坏性系统路径、`$(...)` 子 shell 替换永远都要审批。在 Windows 上若安装了 Git Bash，默认走 Git Bash（找不到则回退 `cmd.exe`）。在 Settings → General 切换。
- **受控 Wiki 覆盖确认** —— 在默认“每次覆盖前确认”模式下，Agent 对已存在 Wiki 页的写入必须经一次性 token、绑定 project 与 session 的待确认写入流程；UI 弹出确认卡显示路径与内容预览，确认后才落地，并产出可撤销的 Agent file activity；切到“本会话直接写入”才能跳过确认
...
### 让 LLM Wiki 作为 MCP Client 连接外部服务（如 MiniMax Token Plan）

LLM Wiki 同时也是**外部 MCP 的 client**：在 **设置 → External MCP** 添加 stdio 服务，让 Agent 直接调用本机的 MCP 工具。

- **MiniMax Token Plan 模板**：一键填充 `command: uvx`、`args: ["minimax-coding-plan-mcp", "-y"]`，并自动建好 `MINIMAX_API_KEY` / `MINIMAX_API_HOST` 环境变量。本机装好 `uvx` 并启用模板后，Agent 即可与内置工具并列调用 `mcp.minimax_token_plan.web_search`。
- **自定义 stdio 服务**：任意本机 MCP 兼容二进制都可填入 command / args / workingDirectory / 各自的 timeoutMs 和 outputCharLimit。
- **带风险提示的启用流程**：启用每个 server 时会弹确认框，明确说明该 server 的所有工具都将被 Agent 自动调用，且凭据会按应用设置方式持久化。
- **每次 Agent run 独立会话**：run 开始时 spawn + initialize + list_tools，结束/取消时统一回收子进程；不在多次 run 之间共享状态。

请求流、安全边界及后续工作见 [`docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md`](docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md)。
