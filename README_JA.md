...
- **外部 MCP Client** — LLM Wiki をローカル stdio MCP サーバー（MCP）のクライアントとして接続。**MiniMax Token Plan テンプレート**（`uvx minimax-coding-plan-mcp -y`）を同梱し、検出されたツールは安定した `mcp.<server>.<tool>` 名で Agent から組み込みツールと同列に呼び出せる（run ごとに独立セッション／タイムアウト／出力切り詰め／子プロセス reaping を実施）
- **ライブ検索と Wiki クイック検索** — 検索ビューを入力即時検索に変更（300ms debounce + 古いレスポンスのフィルタリング）。Wiki プレビューヘッダーにはインラインのクイック検索が追加され、Enter で先頭のサジェストを開き、結果が無ければ Search ビューへ遷移
- **Wiki ページ削除ボタン** — Wiki エディタのツールバーに削除ボタンを追加（Link / Edit の隣）。削除対象は `<project>/wiki/` 直下のみに限定され、成功時にはファイルツリーをリフレッシュしてプレビューを閉じる
...
### 外部 MCP Client として MiniMax Token Plan 等に接続

LLM Wiki は同時に**外部 MCP の Client** としても動作します。**設定 → External MCP** で stdio サーバーを追加すれば、ローカルの MCP ツールを Agent から直接呼び出せます。

- **MiniMax Token Plan テンプレート**: `command: uvx`、`args: ["minimax-coding-plan-mcp", "-y"]`、そして `MINIMAX_API_KEY` / `MINIMAX_API_HOST` 環境変数をワンクリックで入力。本機に `uvx` を入れて有効化すれば、`mcp.minimax_token_plan.web_search` を組み込みツールと同列に呼び出せます。
- **カスタム stdio サーバー**: ローカルの MCP 互換バイナリを登録できます。command / args / workingDirectory と、サーバー単位の timeoutMs / outputCharLimit を設定。
- **リスクを明示する有効化フロー**: サーバー有効化時に確認ダイアログを表示し、検出されたすべてのツールが Agent から自動実行されうること、認証情報がアプリ設定と同じ方式で保存されることを明示します。
- **各 Agent run は独立セッション**: run ごとに spawn → initialize → list_tools、終了 / キャンセル時に子プロセスを reap。run 間で状態を共有しません。

リクエストフロー、安全境界、今後の作業については [`docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md`](docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md) を参照してください。
