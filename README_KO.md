...
- **외부 MCP Client** — LLM Wiki를 로컬 stdio MCP 서버의 클라이언트로 연결합니다. **MiniMax Token Plan 템플릿**(`uvx minimax-coding-plan-mcp -y`)을 기본 제공하며, 발견된 도구는 안정적인 `mcp.<server>.<tool>` 이름으로 Agent가 기본 도구와 동일한 자리에서 호출할 수 있습니다(run 단위 독립 세션, 타임아웃, 출력 잘라내기, 자식 프로세스 reaping 적용)
- **실시간 검색 + Wiki 빠른 검색** — 검색 화면이 입력 즉시 검색으로 변경됨(300ms debounce + 오래된 응답 필터링). Wiki 미리보기 헤더에 인라인 빠른 검색이 추가되어 Enter로 첫 제안을 열며, 결과가 없으면 전체 검색 화면으로 이동합니다
- **Wiki 페이지 삭제 버튼** — Wiki 에디터 툴바에 Link / Edit 사이에 삭제 버튼이 추가되었습니다. 삭제는 `<project>/wiki/` 디렉터리로 한정되며, 성공 시 파일 트리를 새로 고치고 미리보기를 닫습니다
...
### 외부 MCP Client 로 MiniMax Token Plan 등 연결하기

LLM Wiki 는 **외부 MCP 의 Client** 로도 동작합니다. **Settings → External MCP** 에서 stdio 서버를 추가하면 로컬 MCP 도구를 Agent 가 바로 호출할 수 있습니다.

- **MiniMax Token Plan 템플릿**: `command: uvx`, `args: ["minimax-coding-plan-mcp", "-y"]`, 그리고 `MINIMAX_API_KEY` / `MINIMAX_API_HOST` 환경 변수를 한 번 클릭으로 채워 줍니다. 본 머신에 `uvx` 가 깔려 있고 템플릿이 활성화되어 있으면, `mcp.minimax_token_plan.web_search` 를 기본 도구와 같은 자리에서 호출할 수 있습니다.
- **커스텀 stdio 서버**: 로컬의 모든 MCP 호환 바이너리를 등록할 수 있습니다. command / args / workingDirectory 그리고 서버 단위의 timeoutMs / outputCharLimit 를 설정.
- **리스크를 알리는 활성화 흐름**: 서버 활성화 시 확인 대화 상자가 뜨며, 해당 서버의 모든 도구가 Agent 에 의해 자동 실행될 수 있다는 점과 인증 정보가 앱 설정과 동일한 방식으로 저장된다는 점을 명시합니다.
- **Agent run 별 독립 세션**: run 마다 spawn → initialize → list_tools, 종료/취소 시 자식 프로세스를 reap. 여러 run 사이에 상태를 공유하지 않습니다.

요청 흐름, 안전 경계, 향후 작업은 [`docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md`](docs/superpowers/specs/2026-07-16-external-mcp-and-ui-ux.md) 를 참고하세요.
